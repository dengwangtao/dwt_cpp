...
std::vector<User> g_currentUserFriendList; // 好友列表

std::vector<Group> g_currentUserGroupList; // 群组列表


bool isMainMenuRunning = false; // 当前是否为主菜单

sem_t rwsem; // 读写信号量

std::atomic<bool> g_isLoginSuccess{false}; // 是否登录成功
...
int main(int argc, char** argv) {
    ...
    if(argc < 3) {
        cerr << "command invalid! example: " << argv[0] << "[ip] [port]" << endl;
        exit(-1);
    }
    ...
    if(-1 == connect(clientfd, reinterpret_cast<sockaddr*>(&addr), sizeof(addr))) {
    ...
    if (0 != responsejs["errno"].get<int>()) { // 登录失败
        ...
        if (responsejs.contains("offlinemsg"))
        {
            ...
            for (std::string &str : vec)
            {
                ...
                {
                    cout << "群消息[" << js["groupid"] << "]:" << js["time"].get<std::string>() << " [" << js["id"] << "]" << js["name"].get<std::string>()
                            << " said: " << js["msg"].get<std::string>() << endl;
                }
            }
        }
...
void help(int fd = 0, const std::string& str = "");
...
void mainMenu(int clientfd)
{
    ...
    while (isMainMenuRunning) {
        ...
        if (commandHandlerMap.count(command) <= 0) {
            cerr << "invalid input command!" << endl;
            continue;
        }

        // 调用相应命令的事件处理回调，mainMenu对修改封闭，添加新功能不需要修改该函数
        commandHandlerMap[command](clientfd, commandbuf.substr(idx + 1, commandbuf.size() - idx)); // 调用命令处理方法
    }
}
...
// "joingroup" command handler
void joingroup(int clientfd, const std::string& str) {
...
// "logout" command handler
void logout(int clientfd, const std::string& str) {
    json js;
